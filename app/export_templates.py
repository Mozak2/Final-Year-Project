...
class BasicModel:
    def __init__(self, dataset_path, num_classes={num_classes}, img_size={img_size}, channels={channels}, dropout_rate={dropout_rate}, learning_rate={learning_rate}, filters={filters}):
    ...
    def build_VGG16_model(self):
        ...
        input_shape = (self.img_size, self.img_size, self.channels)  # (height, width, channels)
        ...
        model = tf.keras.models.Sequential([
            ...
            # Block 1
            tf.keras.layers.Conv2D(self.filters, (3, 3), activation='relu', padding='same'),
            tf.keras.layers.Conv2D(self.filters, (3, 3), activation='relu', padding='same'),
            ...
            # Block 2
            tf.keras.layers.Conv2D(self.filters * 2, (3, 3), activation='relu', padding='same'),
            tf.keras.layers.Conv2D(self.filters * 2, (3, 3), activation='relu', padding='same'),
            tf.keras.layers.MaxPooling2D((2, 2)),
            tf.keras.layers.Dropout(self.dropout_rate),

            # Block 3
            tf.keras.layers.Conv2D(self.filters * 4, (3, 3), activation='relu', padding='same'),
            tf.keras.layers.Conv2D(self.filters * 4, (3, 3), activation='relu', padding='same'),
            tf.keras.layers.MaxPooling2D((2, 2)),
            tf.keras.layers.Dropout(self.dropout_rate),
...
def main():
    ...
    resnet_model = model.build_VGG16_model()
    resnet_model.fit(train_generator, validation_data=validation_generator, epochs={epochs} )
    resnet_model.save('trained_model.keras')
...
class BasicModel:
    def __init__(self, dataset_path, num_classes={num_classes}, img_size={img_size}, channels={channels}, dropout_rate={dropout_rate}, learning_rate={learning_rate}):
        self.dataset_path = dataset_path
        self.num_classes = num_classes
        self.img_size = img_size
        self.channels = channels
...
def main():
    ...
    model = BasicModel(dataset_path)
    train_generator, validation_generator = model.load_dataset()
    resnet_model = model.build_leNet_model()
